...
export const hiraganaData: KanaRow[] = [
  {
    label: '清音',
    items: [
...
export const katakanaData: KanaRow[] = [
  {
    label: '清音',
    items: [
...
export const hiraganaToKatakanaMap: Record<string, string> = {};
hiraganaData[0].items.forEach((h, i) => {
  const k = katakanaData[0].items[i];
  if (k) hiraganaToKatakanaMap[h.kana] = k.kana;
});
