...
- 完成阶段一全部 + 阶段二的核心功能
- 阅读练习模块集成新闻热点自动抓取
- GitHub Actions 定时任务每日凌晨自动抓取日语新闻
- GitHub Pages 自动部署

## v0.2.0 变更记录

### 新闻阅读系统
- 实现多源日语新闻自动抓取脚本（NHK Easier、NHK速報、Yahoo!ニュース）
- 新闻内容自动难度分级（N1-N5，基于汉字比例和文本长度）
- 新闻数据去重和质量筛选
- 创建 GitHub Actions 工作流（每日凌晨15:00 UTC = 日本时间00:00自动执行）
- 抓取后自动提交数据变更并触发重新部署
- 前端阅读页面新增「新闻热点」标签页
- 新闻支持按级别和来源筛选
- 新闻详情页展示带振假名（ruby标签）的原文内容
- 新闻服务模块（newsService）带本地缓存
